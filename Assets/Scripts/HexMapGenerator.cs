...
public class HexMapGenerator : MonoBehaviour {
    ...
    [Range(0, 1f)] public float highRiseProbability = 0.25f;
        ...
        for (int guard = 0; landBudget > 0 && guard < 10000; guard++) {
            ...
            for (int i = 0; i < regions.Count; i++) {
                ...
                if (sink) {
                    ...
                    landBudget = RaiseTerrain(Random.Range(chunkSizeMin, chunkSizeMax + 1), landBudget, region);
                    if (landBudget == 0) {
                        return;
                    }
                }
    ...
    HexCell GetErosionTarget(HexCell cell) {
        ...
        if (candidates.Count == 0) {
            Debug.LogError("no erosion target found");
        }
    ...
    void CreateRivers() {
        ...
        if (riverBudget > 0) {
            Debug.LogWarning("Failed to use up river budget." + riverBudget);
        }
